...
def create_qr(data):
    ...
    qr.print_ascii(qr.make(fit=True))

    filename = raw_input('Save QR image as: ')
    img = qr.make_image()
    img.save(filename)
...
if __name__ == '__main__':
    ...
    tx_hash = send_message(address, 3, sig, 'IDENTIY9VERIFICATION', node, 0)
    print 'Transfer complete.\n TX hash: ',tx_hash
    print 'Verified by BTC address: ',bitcoinaddress, '\nPlease store for QRCode future reference\n\n'

    data = bitcoinaddress + ' ' + bytes(tx_hash)
    create_qr(data)
